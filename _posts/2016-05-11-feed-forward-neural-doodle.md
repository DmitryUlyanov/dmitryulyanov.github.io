...
A competing approach to the original distribution matching idea of artistic style is a patch-based approach described in the [paper of C. Li](http://arxiv.org/abs/1601.04589). They generate an image $x$ in such a way, that every pixel at each layer $l$ of neural network has a similar one in the style image. This can yield awesome results when the there is no need to preserve the distribution.
